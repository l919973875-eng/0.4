...
## v0.4.4 新增：小红书苗头 Query 自动生成器

小红书不再只依赖 20 条手工 signal 关键词。`config/social_keywords.yaml` 现在定义统一的社交风险 taxonomy：

- 对象：人员、中资项目、工程/能源/矿业央企、重点企业、外交/军事对象、供应链对象；
- 异常动作：人员安全、项目中断、劳资社会、监管/经贸、外交、军事、供应链异常；
- 场景：矿山、港口、工厂、工业园、电站、水坝、铁路、油气田、台海、南海等。

`tools/social_keyword_pack.py` 只按高价值模板生成合法组合，不做全量笛卡尔积；再按权重、类别配额、对象/动作多样性和北京时间日期进行稳定轮换。

当前默认：

- `signal / xhs`：每天最多 **36** 个 query，用于自动巡检；
- `broad / xhs`：最多 **72** 个 query，只建议手动深搜；
- 自动去重；
- 8 条高价值核心词常驻，其余按模板轮换；
- 抖音 / 微博暂时继续使用 v0.4.3 静态词包，待小红书验证后再复制生成逻辑。

社交 Review 的动作/对象/场景识别也会读取同一份 taxonomy，因此发现层与入池筛选层不再维护两套互相漂移的词表。每次社交 Action 还会把当天的小红书 query plan 放进诊断 Artifact，便于复盘哪些词被实际搜索。
